...
check(service.index('journal.baselineText(profile.id,all)') > service.index('boolean delivered=contextWriteIssued&&!prepared'),'Evidence baseline must be captured only after verified context delivery')
