...
check('artemisLiveAgent.nextLive' in service and 'transport.startLive(r)' in service,'Live start is not governed by embedded Artemis')
...
artemis_root=read('app/src/main/java/com/aleyon/geminibridge/artemis/ArtemisRootResolver.java')
check('ArtemisFlashAgent' in service and 'artemisAgent.next' in start_flow,'Embedded Artemis Flash runtime missing from START flow')
...
check(start_flow.find('artemisAgent.next') < start_flow.find('transport.openConversationSearch'),'Search is not gated by a fresh Artemis decision')
...
check('RETRY_BACKOFF_MS' in artemis_root and 'FOCUS_INPUT' in artemis_root,'Artemis root recovery tiers missing')
check('MAX_START_RUNTIME_MS' in service and 'MAX_ROUTE_REPLANS' in service,'Anti-freeze transport watchdog missing')
check('artemisContextAgent.nextContext' in service and 'artemisLiveAgent.nextLive' in service,'Artemis does not govern context delivery and Live end-to-end')
...
check('transport.sendContext(r,intent.contextPayload)' not in start_flow,'Legacy monolithic context-send loop returned to START flow')
check(service.index('artemisContextAgent.nextContext') < service.index('artemisLiveAgent.nextLive'),'Context delivery must complete before Artemis starts Live')
...
check('artemisRenameAgent.nextRename' in service and 'transport.openChatOptions' in service and 'transport.setCanonicalTitle' in service,'Canonical rename is not governed by Artemis')
check('artemisCloseAgent.nextEndLive' in service and 'transport.endLive(r)' in service,'Live close is not governed by Artemis')
check('artemisDebriefAgent.nextContext' in service,'Debrief delivery is not governed by Artemis')
