...
pedagogy=read('app/src/main/java/com/aleyon/geminibridge/core/PedagogicalState.java')
pedagogy_builder=read('app/src/main/java/com/aleyon/geminibridge/core/PedagogicalStateBuilder.java')
...
check('PedagogicalStateBuilder.build(ledger)' in capsule and 'state.compactText(MAX_STATE)' in capsule,'Compact learned state is not feeding the capsule')
check('MAX_PROGRESS=2' in pedagogy_builder and 'MAX_REINFORCEMENT=2' in pedagogy_builder and 'MAX_VOCABULARY=4' in pedagogy_builder,'Pedagogical state limits drifted')
check('count>=2' in pedagogy_builder,'A single reinforcement observation can become a confirmed pattern')
check('pedagogyVersion",2' in store and 'pedagogicalStateJson' in store,'Pedagogical State v2 is not exposed from local evidence')
...
check("no digas «entendido»" in capsule.lower() and 'Máximo una pregunta principal por turno' in capsule,'Tutor prompt still encourages robotic acknowledgement/questionnaires')
check('Intégralo en silencio' in capsule and 'no lo confirmes' in capsule,'Context capsule is not silent/meta-free')
check('Error menor que no impide entender: sigue' in capsule and 'reduce velocidad -> reformula -> simplifica -> da una pista/ejemplo' in capsule,'Compact adaptive correction ladder missing')
