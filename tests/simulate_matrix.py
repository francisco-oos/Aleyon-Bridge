...
def run():
    ...
    total_sessions=0; material_sessions=0; fresh_chats=0; fail_closed=0
    for pi,p in enumerate(profiles):
        for session in range(6):
            ...
            p.memory.events.append(('session-reinforcement',f'reforzar-{p.memory.sessions}'))
            if p.memory.sessions>1 and not before_summary:
                errors.append(f'local continuity lost before next session: {p.lang}')
            if len(p.memory.events)!=2*p.memory.sessions:
                errors.append(f'evidence enrichment drift {p.lang}')
    ...
    print(f'PASS exhaustive contract simulation: profiles={len(profiles)}, sessions={total_sessions}, '
          ...
          f'fail_closed_probes={fail_closed}, version_families={len(VERSION_FAMILIES)}')
    return 0
