...
public final class GeminiUi {
    ...
    /**
     * Opens Gemini's navigation drawer without screen coordinates.
     *
     * Priority:
     *  1) accessibility meaning supplied by Gemini (current field build:
     *     contentDescription="Abrir barra lateral");
     *  2) no guessed fallback: if that semantic control is absent, fail closed.
     *
     * The caller must verify the expected postcondition before continuing.
     */
    public static boolean clickNavigationToggle(AccessibilityNodeInfo root) {
        if (root == null) return false;
        // FIELD BUG (2026-09-07, real device, different phone/Gemini build
        // than every earlier field report this project has on file): the
        // exact same drawer-toggle control can carry a completely different
        // content-desc depending on the Gemini app build/device -
        // "Lista de conversaciones" ("Conversation list") was found here
        // instead of "Abrir barra lateral" ("Open sidebar"), and none of the
        // existing candidates matched it, so the toggle was never found at
        // all - every SETUP/START/CLOSE run failed at the very first step
        // that needs the drawer. Confirms the user's own point: this must
        // never assume one device's/build's exact wording is universal.
        AccessibilityNodeInfo semantic = findAny(root,
                "Abrir barra lateral", "Open sidebar", "Open side navigation",
                "Lista de conversaciones", "Conversation list");
        if (semantic == null) {
            semantic = findContains(root,
                    "Abrir barra lateral", "Open sidebar", "side navigation",
                    "Lista de conversaciones", "Conversation list");
        }
        // Fail closed. Earlier alpha11-derived code guessed the only clickable
        // descendant of the Robin toolbar when the semantic drawer control was
        // missing. On current Nubia/Gemini builds that can open an unrelated
        // Gemini surface (for example an unrelated Gemini configuration surface). Aleyon must
        // never guess a navigation control.
    ...
    /** Rename dialog field. The dialog is expected to expose the only/largest editable control. */
    public static boolean setConversationTitle(AccessibilityNodeInfo root, String title) {
        if (root == null || title == null || title.trim().isEmpty()) return false;
        AccessibilityNodeInfo target = focusedOrOnlyEditable(root);
        return target != null && setText(target, title.trim());
    }

    /** Confirm the rename using a semantic action label. */
    public static boolean saveConversationTitle(AccessibilityNodeInfo root) {
        return clickAnyExact(root, "Guardar", "Save", "Listo", "Done");
    }
    ...
    /**
     *
     * FIELD BUG (2026-09-06, real device): every call site here used to be
     * {@code clickAny(root, "Más", "More")}, which falls back to a substring
     * {@code contains()} match when no exact match exists. Every
     * "ALEYON IDIOMAS — &lt;idioma&gt;", and the Spanish word "idiomas"
     * contains "mas" as a literal substring — so {@code findContains()}
     * on every single retry. Confirmed live with AccessibilityService
     * diagnostics: the matched header node's parent chain had no clickable
     * ancestor within 5 levels (it is a static row inside a RecyclerView), so
     * {@code clickNodeOrClickableParent} always failed and the operation gave
     * up after exhausting its retry budget. This one bug blocked SETUP
     * same call.
     *
     * Fixed with exact match only. "idiomas" can equal "mas" only under
     * substring containment, never under exact string equality, so this can
     * never regress into matching a title again.
     */
    public static boolean clickMoreOptionsMenu(AccessibilityNodeInfo root) {
        // FIELD BUG (2026-09-06, real device): once a chat has at least one
        // Gemini reply, that reply's own per-message action row (thumbs
        // up/down, copy, regenerate, listen, ...) exposes a SECOND, entirely
        // separate button with the exact same content-desc "Más opciones"
        // (resource id suffix
        // assistant_robin_message_content_actions_options_button) - a
        // Share/Report/etc. menu for that one message, not the chat-level
        // breadth-first search can return either one depending on tree
        // order, so clickAnyExact intermittently opened the wrong menu and
        // every caller two steps later failed to find "Cambiar nombre" with
        // no indication anything upstream had gone wrong. The real
        // chat-level toggle carries no resource id at all in the field
        // dump, so exclude the message-row button by its resource id suffix
        // rather than relying on tree order.
        AccessibilityNodeInfo node = findAnyExcludingResourceId(root,
                "assistant_robin_message_content_actions_options_button",
                "Más opciones", "More options", "Más", "More");
        return node != null && clickNodeOrClickableParent(node);
    }

    /** Same exact-match search as findAny, skipping nodes whose resource id contains excludeIdFragment. */
    private static AccessibilityNodeInfo findAnyExcludingResourceId(
            AccessibilityNodeInfo root, String excludeIdFragment, String... candidates) {
        if (root == null) return null;
        List<String> wanted = normalize(candidates);
        Queue<AccessibilityNodeInfo> q = new ArrayDeque<>();
        q.add(root);
        while (!q.isEmpty()) {
            AccessibilityNodeInfo n = q.remove();
            String id = n.getViewIdResourceName();
            boolean excluded = id != null && id.contains(excludeIdFragment);
            if (!excluded && isActionablyVisible(n, root) && !n.isEditable()) {
                String nt = norm(nodeText(n));
                String nd = norm(n.getContentDescription() == null
                        ? "" : n.getContentDescription().toString());
                for (String w : wanted) {
